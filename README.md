...
- A metagraph's `values`/`values2`/`values3` may each reference either a declared scalar value (from `add_values()`) or a resolved setting name (from `add_settings()`, e.g. `lambda`, `mutation_x`), the same domain already used by `xvalue`.
- Graphs and metagraphs support optional `last` (default `false`), `filter`, and `range` fields. `last: true` creates only the final unnumbered graph after all data is available; metagraphs do not create intermediate frames/GIFs, and ordinary graphs do not create annual frames. `filter` uses inclusive numeric `[min, max]` bounds and drops rows failing any listed field. `range` sets only the named axis/normalization range; omitted ranges remain automatic, and plotted X/Y coordinates outside explicit bounds are clamped to the plot edge.
